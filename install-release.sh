...
install_geodata() {
  ...
  local download_link_geoip="https://github.com/Loyalsoldier/geoip/releases/latest/download/geoip-only-cn-private.dat"
  local download_link_geosite="https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat"
  local file_ip_dl='geoip-only-cn-private.dat'
  ...
  download_geodata $download_link_geoip $file_ip_dl
  ...
  install -m 644 "${dir_tmp}"/${file_ip_dl} "${DAT_PATH}"/${file_ip}
  rm -r "${dir_tmp}"
  exit 0
}
